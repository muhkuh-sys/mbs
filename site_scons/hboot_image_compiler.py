...
class RewriteName(ast.NodeTransformer):
	...
	def visit_Name(self, node):
		...
		elif (not self.__atTemporaryConstants is None) and (node.id in self.__atTemporaryConstants):
...
class PatchDefinitions:
	...
	# This is a dictionary of all constants. They are read from the patch definition.
	m_atConstants = None
	...
	def get_patch_definition(self, strOptionId):
		if not strOptionId in self.m_atPatchDefinitions:
	...
	def get_spi_macro_data(self, tDataNode):
		...
		for strRawElement in atRawElements:
			...
			if len(strElement)>0 and strElement[0]!='#':
				...
				if len(atTmp)==1:
					# The line does not contain a colon. This counts as one byte.
					ulAddress += 1
		...
		for tOptionNode in tChunkNode.childNodes:
			...
			if tOptionNode.nodeType==tOptionNode.ELEMENT_NODE and tOptionNode.localName=='Option':
				...
				for iCnt in range(0, sizElements):
					...
					(strElementId, ulSize, ulType) = atElements[iCnt]
					if ulType==0:
#						print 'Type0: %s' % binascii.hexlify(atData[iCnt])
						atOptionData.extend(atData[iCnt])
					elif ulType==1:
#						print 'Type1: len=%02x data=%s' % (sizElement, binascii.hexlify(atData[iCnt]))
						# Add a size byte.
...
class HbootImage:
	...
	def __build_standard_header(self, atChunks):
		...
		# For now only the lower 4 bits are defined. They set the number of hash DWORDs minus 1.
	...
	def __parse_numeric_expression(self, strExpression):	
		...
		# TODO: is this really necessary? Maybe ast.literal_eval throws something already.
		if ulResult is None:
	...
	def __parse_header_options(self, tOptionsNode):
		...
		for tValueNode in tOptionsNode.childNodes:
			if (tValueNode.nodeType==tValueNode.ELEMENT_NODE) and (tValueNode.localName=='Value'):
				# Found a value node. It must have an index attribute which evaluates to a number between 0 and 15.
	...
	def __get_data_contents(self, tDataNode, atData):
		...
		for tNode in tDataNode.childNodes:
			...
			if tNode.nodeType==tNode.ELEMENT_NODE and tNode.localName=='File':
				...
				# Search the file in the current working folder and all include paths.
	...
	def parse_image(self, tInput):
		...
		if len(strType)!=0:
			if not strType in self.__astrToImageType:
...
def main():
	...
	tParser.add_argument('-c', '--objcopy',     dest='strObjCopy',        required=False, default='objcopy', metavar='FILE', help='Use FILE as the objcopy tool.')
	tParser.add_argument('-d', '--objdump',     dest='strObjDump',        required=False, default='objdump', metavar='FILE', help='Use FILE as the objdump tool.')
	tParser.add_argument('-k', '--keyrom',      dest='strKeyRomPath',     required=False, default=None,      metavar='FILE', help='Read the keyrom data from FILE.')
	tParser.add_argument('-p', '--patch-table', dest='strPatchTablePath', required=False, default=None,      metavar='FILE', help='Read the patch table from FILE.')
	tParser.add_argument('-r', '--readelf',     dest='strReadElf',        required=False, default='readelf', metavar='FILE', help='Use FILE as the readelf tool.')
	tParser.add_argument('-v', '--verbose',     dest='fVerbose',          required=False, default=False, action='store_const', const=True, help='Be more verbose.')
	tParser.add_argument('-A', '--alias',       dest='astrAliases',       required=False, action='append',   metavar='ALIAS=FILE', help='Add an alias in the form ALIAS=FILE.')
	tParser.add_argument('-I', '--include',     dest='astrIncludePaths',  required=False, action='append',   metavar='PATH', help='Add PATH to the list of include paths.')
	tParser.add_argument('strInputFile', metavar='FILE', help='Read the HBoot definition from FILE.')
	tParser.add_argument('strOutputFile', metavar='FILE', help='Write the HBoot image to FILE.')
	tArgs = tParser.parse_args()

	# Set the default for the patch table here. This will depend on a chip type later.
	if tArgs.strPatchTablePath is None:
		tArgs.strPatchTablePath = 'hboot_netx4000_patch_table.xml'

	# Parse all alias definitions.
	atKnownFiles = {}
	if not tArgs.astrAliases is None:
	...
	tEnv = {'OBJCOPY': tArgs.strObjCopy, 'OBJDUMP':tArgs.strObjDump, 'READELF':tArgs.strReadElf, 'HBOOT_INCLUDE':tArgs.astrIncludePaths}
