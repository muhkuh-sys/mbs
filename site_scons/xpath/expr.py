...
class AndExpr(BinaryOperatorExpr):
    ...
    def evaluate(self, node, pos, size, context):
        ...
        return (
            xpath.tools.invoke(
                'boolean', node, pos, size, context,
                self.left.evaluate(node, pos, size, context)
            ) and
            xpath.tools.invoke('boolean', node, pos, size, context,
                               self.right.evaluate(node, pos, size, context))
        )
...
class OrExpr(BinaryOperatorExpr):
    ...
    def evaluate(self, node, pos, size, context):
        ...
        return (
            xpath.tools.invoke('boolean', node, pos, size, context,
                               self.left.evaluate(node, pos, size, context)) or
            xpath.tools.invoke('boolean', node, pos, size, context,
                               self.right.evaluate(node, pos, size, context))
        )
...
class EqualityExpr(BinaryOperatorExpr):
    ...
    def operateImpl(self, a, b, node, pos, size, context):
        if xpath.tools.nodesetp(a):
            for node in a:
                if self.operateImpl(
                    xpath.tools.string_value(node),
                    b,
                    node,
                    pos,
                    size,
        ...
        if self.op in ('=', '!='):
            if xpath.tools.booleanp(a) or xpath.tools.booleanp(b):
                convert = 'boolean'
            elif xpath.tools.numberp(a) or xpath.tools.numberp(b):
                convert = 'number'
            else:
                convert = 'string'
        else:
        ...
        a, b = map(lambda x: xpath.tools.invoke(
            convert,
            node,
            pos,
            size,
...
class ArithmeticalExpr(BinaryOperatorExpr):
    ...
    def evaluate(self, node, pos, size, context):
        return self.operators[self.op](
            xpath.tools.invoke('number', node, pos, size, context,
                               self.left.evaluate(node, pos, size, context)),
            xpath.tools.invoke('number', node, pos, size, context,
                               self.right.evaluate(node, pos, size, context))
        )
...
class NegationExpr(Expr):
    ...
    def evaluate(self, node, pos, size, context):
        return -xpath.tools.number(
            self.expr.evaluate(node, pos, size, context),
            context
        )
        ...
        try:
            if self.prefix is not None:
                try:
                    ...
                    raise xpath.exceptions.XPathUnknownPrefixError(self.prefix)
                return context.variables[(namespaceURI, self.name)]
            else:
                return context.variables[(None, self.name)]
        except KeyError:
            raise xpath.exceptions.XPathUnknownVariableError(str(self))
...
class Function(Expr):
    ...
    def evaluate(self, node, pos, size, context):
        ...
        return xpath.tools.invoke(name, node, pos, size, context, *args)
...
class PathExpr(Expr):
    ...
    def evaluate(self, node, pos, size, context):
        ...
        if len(self.steps) > 1 and not xpath.tools.nodesetp(result):
            raise xpath.exceptions.XPathTypeError(
                "path step is not a node-set"
            )
        ...
        for step in self.steps[1:]:
            ...
            for i in range(len(result)):
                ...
                if not xpath.tools.nodesetp(nodes):
                    raise xpath.exceptions.XPathTypeError(
                        "path step is not a node-set"
                    )
...
class PredicateList(Expr):
    ...
    def evaluate(self, node, pos, size, context):
        ...
        for pred in self.predicates:
            match = []
            for i, node in itertools.zip(count(1), result):
                ...
                if xpath.tools.numberp(r):
                    if r == i:
                        match.append(node)
                elif xpath.tools.invoke(
                    'boolean',
                    node,
                    pos,
                    size,
...
class NameTest(object):
    def __init__(self, prefix, localpart):
        ...
        if self.prefix is None and self.localName == '*':
            self.prefix = '*'
