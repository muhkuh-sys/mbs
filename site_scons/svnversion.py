...
def svnversion_action(target, source, env):
    ...
    aSubst = dict({
        ...
        'PROJECT_VERSION': '%s.%s.%s' % (
            version_info[0],
            version_info[1],
            env['PROJECT_VERSION_SVN']
        )
    })
...
def svnversion_emitter(target, source, env):
    ...
    if 'PROJECT_VERSION_SVN' not in env:
    ...
    env.Depends(target, SCons.Node.Python.Value(env['PROJECT_VERSION_SVN']))
...
def ApplyToEnv(env):
    ...
    svnversion_bld = SCons.Script.Builder(
        action=svnversion_act,
        emitter=svnversion_emitter,
        single_source=1
    )
