...
def uuencode_emitter(target, source, env):
    ...
    env.Depends(target, SCons.Node.Python.Value(env['UUE_PRE']))
    env.Depends(target, SCons.Node.Python.Value(env['UUE_POST']))
    env.Depends(target, env['UUE_ELF'])
...
def ApplyToEnv(env):
    ...
    uuencode_bld = SCons.Script.Builder(
        action=uuencode_act,
        emitter=uuencode_emitter,
        suffix='.uue',
        single_source=1,
        src_suffix='.bin'
    )
