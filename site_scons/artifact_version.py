...
def build_artifact_version_strings(env):
    ...
    if 'ARTIFACT_VERSION' not in env:
...
def artifact_version_emitter(target, source, env):
    ...
    env.Depends(target, SCons.Node.Python.Value(env['ARTIFACT_VERSION']))
...
def ApplyToEnv(env):
    ...
    artifact_version_act = SCons.Action.Action(
        artifact_version_action,
        artifact_version_string
    )
    artifact_version_bld = SCons.Script.Builder(
        action=artifact_version_act,
        emitter=artifact_version_emitter,
        single_source=1
    )
