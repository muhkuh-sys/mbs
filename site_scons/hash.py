...
def hash_action(target, source, env):
    ...
    for strHashName in astrHashIDs:
        ...
        try:
            ...
            raise Exception(
                'Unknown hash algorithm "%s". Supported algorithms: %s' %
                (
                    strHashID,
...
def hash_emitter(target, source, env):
    ...
    env.Depends(target, SCons.Node.Python.Value(env['HASH_ALGORITHM']))
    env.Depends(target, SCons.Node.Python.Value(env['HASH_TEMPLATE']))
...
def ApplyToEnv(env):
    ...
    hash_bld = SCons.Builder.Builder(action=hash_act, emitter=hash_emitter)
