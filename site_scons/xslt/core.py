...
def xsltHeader(node, allowedNames=[]):
    ...
    for i in range(node.childNodes.length):
        ...
        if xslt.tools.isWhitespaceNode(child):
            tail.append(child)
...
def xsltChildren(node, allowedNames=[]):
    ...
    for child in children:
        if xslt.tools.isWhitespaceNode(child):
            ...
            raise xslt.exceptions.UnexpectedNode(child)
...
def fixNamespaces(node):
    ...
    if node.nodeType == xml.dom.Node.ELEMENT_NODE:
        ns = xslt.tools.getNamespaceBindings(node)
        ...
        for i in range(node.attributes.length):
            ...
            if(
                attr.namespaceURI not in ns.values() and
                attr.namespaceURI != xml.dom.XMLNS_NAMESPACE
            ):
                node.setAttributeNS(
                    xml.dom.XMLNS_NAMESPACE,
                    'xmlns:' + attr.prefix,
...
def assertEmptyNode(node):
    ...
    if node.childNodes.length > 0:
        raise xslt.exceptions.UnexpectedNode(node.firstChild)
...
class XSLTContext(object):
    ...
    def pushResult(self, value):
        xslt.tools.pushResult(value, self.result)
        return value
...
class TemplateContent(object):
    ...
    def addNodes(self, nodeList, stylesheet, options):
        ...
        for tNode in nodeList:
            if tNode.nodeType == xml.dom.Node.ELEMENT_NODE:
                if tNode.namespaceURI == XSLT_NAMESPACE:
                    if tNode.localName in TemplateContent.allowedElements:
                        e = self._ef.newE(tNode, stylesheet, options)
                    else:
                        if options['forwardsCompatible']:
                            ...
                            print('Element %s not yet implemented!' %
                                  tNode.localName)
                            # TODO: forwards-compatible
                            raise xslt.exceptions.NotImplemented
                else:
                    if tNode.namespaceURI in options[
                        'extensionElementsNS'
                    ]:
                        try:
                            e = self._ef.new(
                        ...
                        except xslt.exceptions.UnexpectedNode:
                            if options['forwardsCompatible']:
                                e = self._ef.new(
                                    '_perform_fallback',
                                    tNode,
            ...
            elif tNode.nodeType == xml.dom.Node.TEXT_NODE:
                e = self._ef.new(
                    '_literal_text',
                    tNode,
                    stylesheet,
...
class ElementFactory(object):
    def new(self, name, element, stylesheet, options):
        ...
        try:
            ...
            raise xslt.exceptions.UnexpectedNode(element)

        return subcls(element, stylesheet, options)

    def newE(self, element, stylesheet, options):
        """Creates an object of approprite Element subclass for an element
        node"""

        return self.new(element.localName, element, stylesheet, options)


class Element(xslt.six.with_metaclass(xslt.tools.ClassInitializer, object)):
    ...
    def __init__(self, element, stylesheet, options):
        ...
        subOptions['namespaces'].update(
            xslt.tools.getNamespaceBindings(element)
        )
        self.namespaces = subOptions['namespaces']

        self.initImpl(element, stylesheet, subOptions)
    ...
    def instantiateContent(self, context):
        """Used by subclasses to instantiate contents compiled by
        setContent."""
    ...
    def instantiate(self, context):
        ...
        self.instantiateImpl(subContext)
