...
class InvalidContent(RuntimeError):
    ...
    def __unicode__(self):
        ...
        return u"%s(%s)" % (type(self).__name__, content)
...
class InvalidName(CompileError):
    ...
    def __unicode__(self):
        return u'InvalidName(%s)' % (self.name)


class DuplicateName(CompileError):
    """Raised if an name was redefined."""

    def __init__(self, name):
        self.name = name

    def __unicode__(self):
        return u'DuplicateName(%s)' % (self.name)
