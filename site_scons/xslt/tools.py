...
try:
    # Try to load the python2 module.
...
def childElements(node):
    ...
    for i in node.childNodes:
        if i.nodeType == xml.dom.Node.ELEMENT_NODE:
            ...
            raise xslt.exceptions.UnexpectedNode(i)
    return elements
...
def isNameTestInList(test, l):
    ...
    and (None, None) allows (None, None), (NCName, None) and
    (NCName, NCName)"""
...
def stripSpace(node, stripSpaceList=[], preserveSpaceList=[],
               defaultStrip=True, stripComments=True, active=True,
               currentActive=True):
    """Removes descendant whitespace nodes according to [XSLT 3.4].
    stripSpace and preserveSpace are lists of NameTests. NameTests from
    preserveSpace take precednce.
    Setting active to False turns off whitespace stripping by default
    (a descendant with xml:space="default" will turn it on).
    Setting currentActive to False turns off whitespace strippng just for
    node itself.
    currentActive is indicating that node's parent is in set of
    whitespace-preserving elements.
    If defaultStrip is True an element is considered whitespace-stripping by
    default (as in XML)
    If defaultStrip is False an element is whitespace-preserving by default
    (as in [XSLT 4.3] for source docs)"""
    ...
    if node.nodeType == xml.dom.Node.TEXT_NODE:
        ...
        if isNameTestInList((node.namespaceURI, node.localName),
                            stripSpaceList):
            currentActive = True
        if isNameTestInList((node.namespaceURI, node.localName),
                            preserveSpaceList):
...
def pushResult(value, result):
    ...
    result can be basestring, Attr, Text, Comment, CDATASection,
    ProcessingInstruction,Element, DocumentFragment, Document.
    ...
    if(
        ...
        if(
            isinstance(value, xml.dom.Node) and
            value.nodeType == xml.dom.Node.TEXT_NODE
        ):
            value = value.data
        ...
        if sys.version_info.major == 2:
            ...
            if not isinstance(value, basestring):
                raise xslt.exceptions.InvalidContent(value)
    ...
    if isinstance(result, xml.dom.Node):
        if(
            ...
            if(
                ...
                if(
                    result.nodeType != xml.dom.Node.ELEMENT_NODE or
                    result.childNodes.length > 0
                ):
                    raise xslt.exceptions.UnexpectedAttribute(value)
...
def combineOutput(output, newvalues):
    ...
    if(
        'cdata-section-elements' in newvalues and
        'cdata-section-elements' in output
    ):
        newvalues['cdata-section-elements'].update(
...
def computeDefaultPriority(pattern):
    ...
    if(
        type(pattern) is X.AbsolutePathExpr and
...
def splitUnionExpr(expr):
    """Split a tree of UnionExpr's to a list of Expr's that are not
    UnionExpr"""
