...
def resolveNamespace(prefix, namespaces, resolveDefault=False):
    ...
    try:
        ...
        raise xslt.exceptions.NamespaceNotFound(prefix, prefix)
...
def parseQName(qname):
    ...
    if m is None:
        raise xslt.exceptions.InvalidName(qname)
...
def floatProperty(el, prop, default=None, required=False):
    ...
    try:
        ...
        raise xslt.exceptions.InvalidAttribute(el, prop, s, e)
...
def qnameProperty(el, prop, namespaces={}, default=None, required=False,
    ...
    try:
        if s is not None:
            s = resolveQName(s, namespaces, resolveDefault)
    except xslt.exceptions.InvalidName as e:
        raise xslt.exceptions.InvalidAttribute(el, prop, e.name, e)

    return s


def qnameListProperty(el, prop, namespaces={}, default=None, required=False,
                      resolveDefault=False):
...
def nameTestListProperty(el, prop, namespaces={}, default=None,
                         required=False, resolveDefault=False):
...
def nsListProperty(el, prop, namespaces={}, default=None, required=False):
    ...
    Raises InvalidAttribute if any prefix can't be resolved,
    or AttributeRequired.
...
def exprProperty(el, prop, default=None, required=False):
    ...
    try:
        s = xslt.xp.XPath(s) if s is not None else None
...
def attributeTemplateProperty(el, prop, default=None, required=False):
    ...
    try:
        s = xslt.xp.AttributeTemplate(s) if s is not None else None
    except xpath.exceptions.XPathParseError as e:
        raise xslt.exceptions.InvalidAttribute(el, prop, s, e)
...
def patternProperty(el, prop, default=None, required=False):
    ...
    try:
        s = xslt.xp.Pattern(s) if s is not None else None
    except xpath.exceptions.XPathParseError as e:
        raise xslt.exceptions.InvalidAttribute(el, prop, s, e)
