...
def getToolAttributes(aCfg, aTool, strPackageMachine):
    ...
    aAttr['PacketDepackPath'] = os.path.join(
        aCfg['depack_path'],
        aTool['group'],
        aTool['name']
    )
    ...
    aAttr['LocalMarkerPath'] = os.path.join(
        aCfg['marker_path'], '%s-%s-%s-%s.marker' % (
            aTool['group'],
            aTool['name'],
            aTool['typ'],
            aTool['version']
    ...
    aAttr['LocalPackageFolder'] = os.path.join(
        aCfg['repository_path'],
        *aPathElements
    )
    aAttr['LocalPackagePath'] = os.path.join(
        aAttr['LocalPackageFolder'],
        strPackageName
    )
...
def install_package(aCfg, aToolAttr):
    if os.path.isdir(aToolAttr['LocalPackageFolder']) is False:
        os.makedirs(aToolAttr['LocalPackageFolder'])

    if os.path.isdir(aToolAttr['PacketDepackPath']) is False:
        os.makedirs(aToolAttr['PacketDepackPath'])

    # Is the package already downloaded?
    # Both the package and the sha1 must exist.
    bDownloadOk = os.path.isfile(aToolAttr['LocalPackagePath']) and \
        os.path.isfile(aToolAttr['LocalSha1Path'])

    if bDownloadOk is True:
        print('The package was already downloaded, check the files.')
        # Check the sha1 sum.
        bDownloadOk = check_sha1_sum(
            aToolAttr['LocalSha1Path'],
            aToolAttr['LocalPackagePath']
        )
        if bDownloadOk is True:
    ...
    if bDownloadOk is False:
        ...
        for strRepositoryUrl in aCfg['repositories']:
            ...
            strPackageUrl = strRepositoryUrl
            strPackageUrl += '/'.join(aToolAttr['PathElements'])
            strPackageUrl += '/' + aToolAttr['PackageName']
            strSha1Url = strPackageUrl + '.sha1'

            bDownloadOk = download.download_to_file(
                strSha1Url,
                aToolAttr['LocalSha1Path']
            )
            if bDownloadOk is True:
                ...
                if bDownloadOk is True:
                    ...
                    if bDownloadOk is True:
                        ...
                        print(
                            'Checksum mismatch, discarding downloaded files!'
                        )
                        os.remove(aToolAttr['LocalPackagePath'])
                        os.remove(aToolAttr['LocalSha1Path'])
...
atMachineFallbacks = dict({
    'x86': ['x86', 'i686', 'i586', 'i486', 'i386'],
    'x86_64': ['x86_64', 'amd64', 'i686', 'i586', 'i486', 'i386']
})


def process_package(aCfg, aTool):
    print('Processing package %s, version %s' % (
        aTool['name'],
        aTool['version']
    ))
    ...
    if strPackage.find('${machine}') > -1:
        # Get the machine name and possible alternatives.
        tPlatform = PlatformDetect()
        tPlatform.detect()
        strMachineName = tPlatform.strHostCpuArchitecture
        ...
        for strMachineName in astrMachines:
            ...
            strPackageMachine = tTemplate.safe_substitute(
                dict({'machine': strMachineName})
            )
            ...
            if os.path.isfile(aToolAttr['LocalMarkerPath']) is True:
                # Use this package.
                print('Found package!')
                break
            else:
    ...
    if aToolAttr is None:
        raise Exception('Failed to install package %s, version %s' % (
            aTool['name'],
            aTool['version']
        ))
